...
public class GenSpring {
	...
	public GenSpring() throws IOException {
		...
		baseDir = getProp(bundle, propKey + ".outdir");
		schema = getProp(bundle, propKey + ".schema");
		basePkg = getProp(bundle, propKey + ".pkg") + '.' + getProp(bundle, propKey + ".module");
	 ...
	 */
	private String getProp(ResourceBundle bundle, String key) {
	...
	private String getProp(ResourceBundle bundle, String key, String defaultVal) {
		try {
			...
			LOGGER.warn(key + " undefined in " + propKey + " using " + defaultVal);
		}
	...
	/**
	 * Lower case objName then remove each underscore, slash and space then upper
	 * the char right after.
	 * 
	 * @param objName
	 * @return String in camel case
	 */
	private String tabToStr(String objName) {
		if (objName == null) {
			return null;
		}
		try {
			return renames.getString(objName);
		} catch (MissingResourceException e) {

			StringBuffer sb = new StringBuffer(16);
			StringTokenizer st = new StringTokenizer(objName.toLowerCase(), "_ /", false);
...
//			ps.println("        this." + fieldName + "Services = " + fieldName + "Services;");
	...
	private void writeSecurityConfiguration() {
		...
		try (PrintStream ps = new PrintStream(p.toFile())) {
			...
			ps.println("import org.springframework.security.config.annotation.web.configuration.EnableWebSecurity;");
			ps.println(
	...
	private void writeLoginHtml() {
		...
		try (PrintStream ps = new PrintStream(p.toFile())) {
			...
			ps.println("<head>");
			ps.println("    <meta charset=\"UTF-8\"/>");
			ps.println("    <title>" + getProp(bundle, propKey + ".module") + " | Login</title>");
			ps.println("</head>");
			ps.println("<body>");
	...
	private void writeEditPage(String clsName, TreeMap<String, ColInfo> namList, ColInfo pkinfo) {
		...
		try (PrintStream ps = new PrintStream(p.toFile())) {
			...
			ps.println("		<h1 th:if=\"${" + fieldName + "." + pkinfo.getVName() + " == 0}\">Create New " + clsName
					+ "</h1>");
			ps.println("		<h1 th:if=\"${" + fieldName + "." + pkinfo.getVName() + " > 0}\">Edit " + clsName
					+ "</h1>");
			ps.println("		<br />");
			ps.println("		<form action=\"#\" th:action=\"@{/" + fieldName + "s/save}\" th:object=\"${" + fieldName
					+ "}\"");
			ps.println("			method=\"post\">");
			ps.println("");
			ps.println("			<table border=\"1\" cellpadding=\"10\">");
			ps.println(
					"				<tr th:if=\"${" + fieldName + "." + pkinfo.getVName() + " > 0}\">				");
			...
			while (it.hasNext()) {
				...
				if (!pkinfo.getColName().equals(info.getColName())) {
					...
					ps.println("					<td>");
					ps.println(
							"						<input type=\"text\" th:field=\"*{" + info.getVName() + "}\" />");
					ps.println("					</td>");
					ps.println("				</tr>");
				}
			}
	...
	private void writeListPage(String clsName, TreeMap<String, ColInfo> namList, ColInfo pkinfo) {
		...
		try (PrintStream ps = new PrintStream(p.toFile())) {
			...
			ps.println("<head>");
			ps.println("    <meta charset=\"UTF-8\"/>");
			ps.println("    <title>" + getProp(bundle, propKey + ".module") + " | " + clsName + " List View</title>");
			...
			while (it.hasNext()) {
				ColInfo info = (ColInfo) namList.get(it.next());
				ps.println("	            <td th:text=\"${" + fieldName + "." + info.getVName() + "}\"></td>");
			}
	...
	private void writeObjController(String clsName, ColInfo pkinfo) {
		...
		try (PrintStream ps = new PrintStream(p.toFile())) {
			...
			ps.println("		model.addAttribute(\"" + fieldName + "s\", this." + fieldName + "Service.listAll());");
...
//			ps.println("");
	...
	private void writeAppProps() {
		String dbUrl = getProp(bundle, "db.url", "");
		...
		try (PrintStream ps = new PrintStream(p.toFile())) {
			...
			if (dbUrl.contains("sqlite")) {
				...
				ps.println("#spring.datasource.username=" + getProp(bundle, "user", null));
				ps.println("#spring.datasource.password=" + getProp(bundle, "password", null));
			}
	...
	private void writeSQLiteDialect(String pkgNam) {
		...
		try (PrintStream ps = new PrintStream(p.toFile())) {
			...
			ps.println("        registerFunction(\"mod\", new SQLFunctionTemplate(StringType.INSTANCE, \"?1 % ?2\"));");
			ps.println(
			...
			ps.println("    return new StringBuffer(insertString.length()+30). // As specify in NHibernate dialect");
	...
	private void writePom() {
		...
		try (PrintStream ps = new PrintStream(p.toFile())) {
			...
			ps.println("	<groupId>" + getProp(bundle, propKey + ".pkg") + "</groupId>");
			ps.println("	<artifactId>" + getProp(bundle, propKey + ".module") + "</artifactId>");
	...
	private void writeBean(String viewName, String className, TreeMap<String, ColInfo> namList, String create) {
		...
		try (PrintStream ps = new PrintStream(p.toFile())) {
			...
			if (beanToString) {
				...
				while (it.hasNext()) {
					ColInfo info = (ColInfo) namList.get(it.next());
					ps.println("		sb.append(\"" + info.getVName() + "= \" + " + info.getVName() + "+\'\\n\');");
				}
